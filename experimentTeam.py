...
class FlexibleAgent(CaptureAgent):
    def registerInitialState(self, gameState):
        ...
        self.retreatingPoint = None
        self.lastGhostSeen = {}
        self.closestBorder = None
    ...
    def chooseAction(self, gameState):
        ...
        # currGhostStates = [e[1] for e in self.getEnemies(gameState) if not e[1].isPacman and self.retrieveMazeDistance(ourAgent.getPosition(),e[1].getPosition()) <= self.detectDistance]
        ...
        if self.isOffensive:
            ...
            ghostNearby = False
            if opponentInsightIndices:
                for idx in opponentInsightIndices:
                    if gameState.getAgentState(idx).scaredTimer < 10:
                        ghostNearby = True
                        # action = expectimax(gameState, 0, 0)[1]
                        # print("ghost nearby")
            if self.isRetreating or ghostNearby:
                action = expectimax(gameState, 0, 0)[1]
                # print("ghost")
            else:
                if len(currFood) > self.foodLimit:
                    nextFood = self.getNextFood(gameState, currPos)
                    # idx = 0
                    # print(self.checkLastGhostSeen(nextFood[idx][1]))
                    # while self.checkLastGhostSeen(nextFood[idx][1]):
                        # print("cahnge food")
                        # idx += 1
                    # print(idx)
                    paths = self.ucs(gameState, currPos, nextFood[0][1])
                    action = paths[0] if paths else "Stop"
                else:
                    if self.closestBorder is None:
                        self.closestBorder = min([(self.retrieveMazeDistance(currPos, border), border) for border in self.borders])[1]
                    action = self.ucs(gameState, currPos, self.closestBorder)[0]
                    if not action:
                        action = "Stop"
            if len(self.actionHistory.list) >= self.actionHistoryLimit:
    ...
    def evaluateFunction(self, currentGameState):
        ...
        if self.isOffensive:
            ...
            closestCapsuleDistance, closestCapsule = min([(self.retrieveMazeDistance(currPos, capsule), capsule) for capsule in currCapsules]) if currCapsules else (1, None)
            closestFoodDistance, closestFood = min([(self.retrieveMazeDistance(currPos, food), food) for food in currFood]) if currFood else (0, None)
            # def getFindingFoodEval():
            #     evalScore = 0
            #     evalScore -= closestFoodDistance
            #     evalScore -= 100 * len(currFood)
            #     return evalScore
            def getRetreatingEval():
                if self.isRetreating:
                    evalScore = 0
                    if self.retrieveMazeDistance(currPos, self.initPos) > 0:
                        evalScore = 100.0 / self.retrieveMazeDistance(currPos, self.initPos)
                    return evalScore
                else:
                    return None
            ...
            #             ghostNearby = True
                ...
                #     print("ghost")
                #     score -= sys.maxsize
                    # print(3000 / closestBorderDistance if closestBorderDistance > 0 else 3000)
                    # score += 3000 / closestBorderDistance if closestBorderDistance > 0 else 3000
                    # score -= 1000 / closestGhostDistance
                    # if closestCapsule:
                    #     distanceFromGhostToCapsule = self.retrieveMazeDistance(closestGhost, closestCapsule)
                    #     if distanceFromGhostToCapsule > closestCapsuleDistance:
                    #         score -= 100 * closestCapsuleDistance
                    #         score += 3000 * closestBorderDistance
    ...
    def checkLastGhostSeen(self, position, checkPositionAdj=False):
        for key in self.lastGhostSeen:
            ghostPos = self.lastGhostSeen[key]
            if checkPositionAdj:
                if self.checkPositionNearGhost(position, ghostPos):
                    return True
            else:
    ...
    def getNextFood(self, gameState, position):
        currFood = self.getFood(gameState).asList()
        sortedDisFoodList = [(self.retrieveMazeDistance(position, food), food) for food in currFood]
        sortedDisFoodList.sort(key=lambda tup: tup[0])
        return sortedDisFoodList

    def checkPositionNearGhost(self, position, ghostPosition):
        for x in range(-1, 2):
            for y in range(-1, 2):
                if position == (ghostPosition[0] + x, ghostPosition[1] + y):
                    return True
        return False

    def getSuccessorPosition(self, gameState, position):
        successors = []
        opponentInsightIndices = [e[0] for e in self.getEnemies(gameState) if not e[1].isPacman and self.retrieveMazeDistance(position, e[1].getPosition()) <= self.detectDistance]
        for idx in opponentInsightIndices:
            if gameState.getAgentState(idx).scaredTimer < 10:
                self.lastGhostSeen[idx] = gameState.getAgentPosition(idx)
        if not gameState.hasWall(position[0] + 1, position[1]) and not self.checkLastGhostSeen((position[0] + 1, position[1])):
            successors.append(((position[0] + 1, position[1]), "East"))
        if not gameState.hasWall(position[0] - 1, position[1]) and not self.checkLastGhostSeen((position[0] - 1, position[1])):
            successors.append(((position[0] - 1, position[1]), "West"))
        if not gameState.hasWall(position[0], position[1] + 1) and not self.checkLastGhostSeen((position[0], position[1] + 1)):
            successors.append(((position[0], position[1] + 1), "North"))
        if not gameState.hasWall(position[0], position[1] - 1) and not self.checkLastGhostSeen((position[0], position[1] - 1)):
            successors.append(((position[0], position[1] - 1), "South"))
        return successors

    def isGoalState(self, position, foodList, end=None):
        if end:
            return position == end
        return len(foodList) == self.foodLimit

    def ucs(self, gameState, start, end):
        if start == end:
            return ["Stop"]
        ...
        while not frontier.isEmpty():
            ...
            if current == end:
                # print("from ucs:" + str(result_path[0]))
                return result_path
            if current not in visited_states:
                visited_states.append(current)
                for successor in self.getSuccessorPosition(gameState, current):
                    if successor[0] not in visited_states:
                        frontier.push((successor[0], result_path + [successor[1]]), self.getCostOfActions(gameState, start, result_path + [successor[1]]))
        return ["Stop"]  # should not reach this
